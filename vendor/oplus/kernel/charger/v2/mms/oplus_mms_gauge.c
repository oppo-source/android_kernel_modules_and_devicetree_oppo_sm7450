...
static int oplus_mms_gauge_push_vbat_diff_over_msg(struct oplus_mms_gauge *chip,
						   int vbat_max, int vbat_min)
{
	struct oplus_mms *err_topic;
	struct oplus_mms *comm_topic;
	struct mms_msg *msg;
	int rc;
	int soc, ui_soc, batt_status;
	union mms_msg_data data = { 0 };
	...
	rc = oplus_mms_get_item_data(comm_topic, COMM_ITEM_BATT_STATUS, &data, false);
	if (rc < 0) {
		chg_err("can't get batt_status, rc=%d\n", rc);
	...
	msg = oplus_mms_alloc_str_msg(
		MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, ERR_ITEM_VBAT_DIFF_OVER,
		"$$soc@@%d$$$uisoc@@%d$$vbatt_max@@%d$$vbatt_min@@%d$"
		"$vbatt_diff@@%d$$charger_exist@@%d$$type@@%s",
		soc, ui_soc, vbat_max, vbat_min, (vbat_max - vbat_min),
		(chip->wired_online || chip->wls_online),
		(batt_status == POWER_SUPPLY_STATUS_FULL) ? "full" : "shutdown");
	if (msg == NULL) {
		chg_err("alloc vbat_diff_over msg error\n");
		return -ENOMEM;
	}

	rc = oplus_mms_publish_msg(err_topic, msg);
	if (rc < 0) {
		chg_err("publish vbat_diff_over msg error, rc=%d\n", rc);
		kfree(msg);
	}
	...
	if (abs(vbat_max - vbat_min) > ALLOW_DIFF_VALUE_MV) {
		...
		if (chip->check_batt_vol_count > BATT_VOL_ERR_COUNT_MAX) {
			...
			if (!chip->bat_volt_different) {
				...
				oplus_mms_gauge_push_vbat_diff_over_msg(
					chip, vbat_max, vbat_min);
			}
