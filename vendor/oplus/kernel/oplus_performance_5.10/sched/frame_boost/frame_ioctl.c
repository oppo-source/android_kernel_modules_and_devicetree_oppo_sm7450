...
/* Boost param for default frame group */
int stune_boost[BOOST_MAX_TYPE];
/* Boost param for rt union group */
int rt_stune_boost[BOOST_MAX_TYPE];
...
static long ofb_sys_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case CMD_ID_SET_SF_MSG_TRANS:
		...
		if (data.stage == BOOST_CLIENT_COMPOSITION) {
			if (data.level) {
				rt_stune_boost[BOOST_MIGR] = data.boost_migr;
				rt_stune_boost[BOOST_FREQ] = data.boost_freq;
			} else {
				rt_stune_boost[BOOST_MIGR] = 0;
				rt_stune_boost[BOOST_FREQ] = 0;
			}
		}
