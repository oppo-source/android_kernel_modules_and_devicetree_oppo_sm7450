...
static int _sde_connector_update_panel_level(struct sde_connector *c_conn)
{
	...
	bl_config->bl_level = c_conn->panel_bl;
	if (c_conn->unset_bl_level)
		bl_config->bl_level = c_conn->unset_bl_level;
				...
				struct drm_connector *connector)
{
	...
	while ((idx = msm_property_pop_dirty(&c_conn->property_info,
		...
		case CONNECTOR_PROP_PANEL_LEVEL:
			if (c_conn)
				c_conn->update_panel_level = true;
			break;
#endif
		default:
		...
		uint64_t val)
{
	...
	case CONNECTOR_PROP_PANEL_LEVEL:
		 ...
		 */
		c_conn->panel_bl = val & 0x3FFF;
		c_conn->bl_sync_dly = ((val >> 14) & 0x3FFF) * 100;
		c_conn->wait_vsync_flag = (val >> 28) & 0x7;
		c_conn->panel_bl_dirty = true;
		c_conn->update_panel_level = false;
		SDE_DEBUG("panel_bl: %d, delay: %d, wait_vsync_flag: %d\n",
				c_conn->panel_bl, c_conn->bl_sync_dly,
				c_conn->wait_vsync_flag);
		msm_property_set_dirty(&c_conn->property_info,
				&c_state->property_state, idx);
		break;
#endif
...
int sde_connector_update_panel_level(struct sde_connector *c_conn)
{
	...
	now_ktime = ktime_get();
	delta_us = ktime_to_us(ktime_sub(now_ktime, c_conn->rd_ptr_ktime));
	if (c_conn->bl_sync_dly > delta_us) {
		delay = c_conn->bl_sync_dly - delta_us;
