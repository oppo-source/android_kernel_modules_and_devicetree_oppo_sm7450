...
void sde_encoder_wait_vblack(struct drm_connector *connector, struct drm_encoder *drm_enc, int wait_num)
{
	...
	if (c_conn->wait_vsync_flag > 0 && wait_num > 0) {
		if (c_conn->panel_bl_dirty && c_conn->update_panel_level) {
...
void sde_encoder_post_kickoff_update_panel_level(struct drm_connector *connector)
{
	...
	if (c_conn->wait_vsync_flag == 0 || c_conn->wait_vsync_flag == 1) {
		if (c_conn->panel_bl_dirty && c_conn->update_panel_level) {
			sde_connector_update_panel_level(c_conn);
			c_conn->update_panel_level = false;
			c_conn->panel_bl_dirty = false;
		} else if (c_conn->panel_bl_dirty) {
			SDE_ERROR("Panel not send by drm update_panel_level: %d\n",
					c_conn->update_panel_level);
		}
	}
}
...
void sde_encoder_update_panel_level(struct drm_connector *connector,  struct drm_encoder *drm_enc)
{
	...
	if (c_conn->panel_bl_dirty && c_conn->update_panel_level) {
		sde_connector_update_panel_level(c_conn);
		if (c_conn->wait_vsync_flag > 1 && c_conn->wait_vsync_flag < 8) {
			int cnt = c_conn->wait_vsync_flag - 1;
