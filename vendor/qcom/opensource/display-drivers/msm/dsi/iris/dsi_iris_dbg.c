...
int iris_debug_display_mode_get(char *kbuf, int size)
{
	struct iris_cfg *pcfg = iris_get_cfg();
	...
	len += snprintf(kbuf + len, size - len,
			"%-20s:\t%d x %d\n", "Iris mv res", pcfg->frc_setting.mv_hres, pcfg->frc_setting.mv_vres);
		...
		size_t count, loff_t *ppos)
{
	...
	size = iris_debug_display_mode_get(kbuf, size);
	if (size >= count)
		size = count - 1;
