...
void iris_frc_mif_reg_set(void)
{
	...
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_MVC_CTRL_0, 0x58, 0);
...
void iris_mvf_reg_set(void)
{
	if (iris_low_latency_mode_get()) {
		...
		iris_frc_reg_add(IRIS_MVF_ADDR + MVF_SHDW_CTRL, 0x00000017, 0);
		iris_frc_reg_add(IRIS_MVF_ADDR + MVF_SHDW_CTRL, 0x00000011, 0);
	}
}
...
void iris_ai_dma_rd_addr_switch(bool frc)
{
	if (frc) {
		iris_frc_reg_add(IRIS_AI_ADDR + AI_DMA_RD_START3, 0xf2140250, 0);
		iris_frc_reg_add(IRIS_AI_ADDR + AI_DMA_WR_START3, 0xf20102b4, 0);
	} else {
		iris_frc_reg_add(IRIS_AI_ADDR + AI_DMA_RD_START3, 0xf1740060, 0);
		iris_frc_reg_add(IRIS_AI_ADDR + AI_DMA_WR_START3, 0xf174006c, 0);
	}
	iris_frc_reg_add(IRIS_AI_ADDR + AI_UPDATE, 0x2, 0);
}
...
void iris_fi_reg_set(void)
{
	...
	iris_frc_reg_add(IRIS_FI_ADDR + FI_DEMO_MODE_CTRL, 0x00004022 | pcfg->frc_label << 7
			| (pcfg->frc_demo_window ? 1:0), 0);// set flim label
...
void iris_frc_vfr_threshold_set(int count)
{
	u32 write_data[4];
...
void iris_parse_frc_dec_initial_delay(struct device_node *np)
{
	...
	ret = of_property_read_u32(np, "pxlw,frc-dsc-init-delay", &val);
	if (ret != 0) {
...
void iris_memc_info_update(void)
{
	...
	IRIS_LOGI("memc info: mode-%d, mv-%dx%d, level-%d, ratio-%d-%d, vfr-%d, tnr-%d, lt-%d, osdwin-%d",
		...
		pcfg->memc_info.low_latency_mode, pcfg->memc_info.osd_window_en);
}
...
void iris_frc_setting_update(void)
{
	...
	IRIS_LOGI("frc setting: disp-%dx%d@%d, mv-%dx%d coef-%d, buf-%d@%x pps-%d sr_sel-%d dsc-%x/%x",
		...
		frc_setting->pps_table_sel, frc_setting->sr_sel,
...
void iris_memc_ctrl_frc_prepare(void)
{
	...
	iris_mvf_reg_set();
	/* AI */
	iris_ai_dma_rd_addr_switch(true);
	/* FI */
	iris_fi_reg_set();
...
void iris_memc_ctrl_pt_frc_grcp_set(bool frc_enable)
{
	...
	cmd[3] = payload[0];
	cmd[4] = IRIS_PWIL_ADDR + PWIL_DISP_CTRL1;
	cmd[5] = 0x14000000;
	cmd[6] = IRIS_PWIL_ADDR + PWIL_REG_UPDATE;
	cmd[7] = 0x00000100;
...
void iris_memc_ctrl_pt_post(void)
{
	...
	/* peaking_gain = default value(16) */
	iris_sr_setting_reconfigure(16, 0);

	/* power domain off */
	if (!iris_pt_sr_enable)
...
void iris_memc_ctrl_frc_to_pt(void)
{
	...
	/* AI */
	iris_ai_dma_rd_addr_switch(false);
...
bool iris_memc_vfr_setting_update(struct iris_cfg *pcfg, bool enable)
{
	...
	if (enable)
		iris_frc_vfr_threshold_set(iris_fi_drop_frm_thr);
	else
		iris_frc_vfr_threshold_set(0);
...
void iris_memc_vfr_update_work(struct work_struct *work)
{
	...
	if (atomic_read(&pcfg->video_update_wo_osd) >= 4) {
		if (iris_memc_vfr_setting_update(pcfg, true)) {
			pcfg->iris_frc_vfr_st = true;
			IRIS_LOGI("enable vfr");
		}
	} else {
		if (iris_memc_vfr_setting_update(pcfg, false)) {
			pcfg->iris_frc_vfr_st = false;
...
int iris_debug_memc_option_get(char *kbuf, int size)
{
	...
	len += snprintf(kbuf, size,
			"%s: %x\n", "memc_disable_vfr_dual", debug_disable_vfr_dual);
	len += snprintf(kbuf + len, size - len,
			"%s: %x\n", "memc_vfr_enable_ow", debug_frc_vfr_overwrite);
	len += snprintf(kbuf + len, size - len,
			"%s: %x\n", "memc_disable_frc_dynen", debug_disable_frc_dynen);
	len += snprintf(kbuf + len, size - len,
			"%s: %x\n", "memc_mcu_enable", iris_mcu_enable);
	len += snprintf(kbuf + len, size - len,
			"%s: %x\n", "memc_disable_mcu_check", debug_disable_mcu_check);
	len += snprintf(kbuf + len, size - len,
			"%s: %x\n", "memc_switch_dump_clear", debug_switch_dump_clear);
	len += snprintf(kbuf + len, size - len,
			"%s: %x\n", "memc_dma_disable", iris_frc_dma_disable);
	len += snprintf(kbuf + len, size - len,
			"%s: %x\n", "memc_tnr_enable_ow", debug_tnr_en_overwrite);
	len += snprintf(kbuf + len, size - len,
			"%s: %x\n", "memc_layer_c_enable", iris_frc_layer_c_enable);
	len += snprintf(kbuf + len, size - len,
			"%s: %x\n", "memc_fi_drop_frm_thr", iris_fi_drop_frm_thr);
	len += snprintf(kbuf + len, size - len,
			"%s: %x\n", "memc_sr_enable", iris_sr_enable);
	len += snprintf(kbuf + len, size - len,
			"%s: %x\n", "memc_low_latency_ow", debug_low_latency_overwrite);
	len += snprintf(kbuf + len, size - len,
			"%s: %x\n", "memc_3buffer_low_latency", iris_three_buffer_low_latency);
	len += snprintf(kbuf + len, size - len,
			"%s: %x\n", "memc_mv_res_ow", debug_mv_res_overwrite);
	len += snprintf(kbuf + len, size - len,
			"%s: %x\n", "memc_disable_mipi1_autorefresh", debug_disable_mipi1_autorefresh);
...
void iris_frc_ioinc_change(void)
{
	...
	frc_ioinc1d_config(iris_frc_in1_height_get(), iris_frc_in1_width_get(),
			...
			tap_sel, IRIS_IP_IOINC1D, frc_ioinc_sel, 1);
	...
	frc_ioinc1d_config(iris_frc_in2_height_get(),
			iris_frc_in2_width_get(),
			pcfg->frc_setting.disp_hres,
			pcfg->frc_setting.disp_vres,
			tap_sel, IRIS_IP_IOINC1D_2, frc_ioinc_sel, chain);
...
void iris_sr_change(int inHeight, int inWidth, int outHeight, int outWidth, bool sr_sel)
{
	...
	int PREC = 19;
	int PeakingGain = sr_sel ? 16 : 0;	// SR_SEL[PT]: 1, SR_SEL[MEMC]: 0
	...
	//sr
	payload = iris_get_ipopt_payload_data(IRIS_IP_SR, 0xf0, 2);
	if (sr_sel) {
	...
	payload[14] = (payload[14] & 0x00ffffff) | (PeakingGain << 24);

	iris_init_update_ipopt_t(IRIS_IP_SR, 0xf0, 0xf0, chain);
...
void iris_sr_setting_reconfigure(int PeakingGain, bool commit)
{
	uint32_t *payload = NULL;
	//struct iris_cfg *pcfg = iris_get_cfg();
	...
	if (1) {
		payload = iris_get_ipopt_payload_data(IRIS_IP_SR, 0xf0, 2);
		payload[14] = (payload[14] & 0x00ffffff) | (PeakingGain << 24);
		iris_init_update_ipopt_t(IRIS_IP_SR, 0xf0, 0xf0, !commit);
...
{
	int iris_pt_sr_width;
	int iris_pt_sr_height;
	int tap_sel = 5;
	int pt_scale_sel = 1;	// TODO
	struct iris_cfg *pcfg = iris_get_cfg();

	iris_pt_sr_enable = enable & 0x1;
	if (enable & 0x1) {
		iris_pt_sr_enable = true;
		iris_pt_sr_width = processWidth;
		iris_pt_sr_height = processHeight;
		iris_sdr2hdr_update_size2(iris_pt_sr_width, iris_pt_sr_height);
	} else {
		iris_pt_sr_enable = false;
		iris_pt_sr_width = pcfg->frc_setting.disp_hres;
		iris_pt_sr_height = pcfg->frc_setting.disp_vres;
	}

	iris_sr_pwil_update(iris_pt_sr_enable, iris_pt_sr_width, iris_pt_sr_height, 1);

	// iris_update_pq_opt update in below iris_sr_change
	frc_ioinc1d_config(pcfg->frc_setting.disp_vres,
			pcfg->frc_setting.disp_hres,
			iris_pt_sr_width, iris_pt_sr_height,
			tap_sel, IRIS_IP_IOINC1D, pt_scale_sel, 1);

	iris_sr_change(iris_pt_sr_height,
		iris_pt_sr_width,
		pcfg->frc_setting.disp_vres,
		pcfg->frc_setting.disp_hres,
