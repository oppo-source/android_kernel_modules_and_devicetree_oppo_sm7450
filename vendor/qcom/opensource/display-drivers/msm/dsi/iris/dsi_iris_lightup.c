		...
		struct iris_cfg *pcfg)
{
	...
	rc = of_property_read_u8_array(np, "pxlw,virtual-channel-enable", vals, VC_ST_MAX);
	if (rc)
		...
		struct device_node *np, struct iris_cfg *pcfg)
{
	...
	rc = of_property_read_u8_array(np, "pxlw,dsi-trans-mode", arr_8, 2);
	if (rc)
	...
	arr = of_get_property(np, "pxlw,dsi-trans-len", &length);
	if (!arr) {
	...
	rc = of_property_read_u32_array(np, "pxlw,dsi-trans-len",
					arr_32, length);
	if (rc) {
...
static void __cont_splash_work_handler(struct work_struct *work)
{
	...
	// do {
	//	done = iris_abyp_switch_proc(pcfg->display, ANALOG_BYPASS_MODE);
	//	if (done)
	//		done = iris_abyp_switch_proc(pcfg->display, PASS_THROUGH_MODE);
	// } while (!done);
}
...
int iris_parse_param(struct dsi_display *display)
{
	...
	if (rc) {
		IRIS_LOGE("%s(), failed to parse split pkt info, using default info", __func__);
	}
		...
		struct iris_cmd_comp *pcmd_comp)
{
	...
	IRIS_LOGD("%s, op_type: %s, dsi_trans_mode: %d", __func__,
...
static void _iris_pre_timing_config(void)
{
	...
	//sys pll
	iris_update_ip_opt(IRIS_IP_SYS, 0xA0, 0x1);
	//dtg
	iris_update_ip_opt(IRIS_IP_DTG, 0x0, 0x1);
	//mipi tx
	iris_update_ip_opt(IRIS_IP_TX, 0x0, 0x1);
	//mipi abyp
	len = iris_update_ip_opt(IRIS_IP_RX, 0xE0, 0x0);
	iris_update_pq_opt(PATH_DSI, false);
}
...
void _iris_read_power_mode(struct dsi_panel *panel)
{
	...
	struct dsi_cmd_desc cmds = {
		...
		sizeof(get_power_mode), get_power_mode, 1, read_cmd_rbuf}, 1, 0, 0, DSI_CTRL_CMD_READ};
...
int iris_enable(struct dsi_panel *panel, struct dsi_panel_cmd_set *on_cmds)
{
	...
	if ((iris_platform_get() != 0) && !(iris_lightup_opt_get() & 0x2)) {
...
static void _iris_send_cont_splash_pkt(uint32_t type)
{
	...
	if (type == IRIS_CONT_SPLASH_LK) {
		...
		iris_lp_enable_pre();
		pcfg->iris_initialized = false;
		pt_mode = iris_abyp_switch_proc(pcfg->display, PASS_THROUGH_MODE);
		if (pt_mode)
