...
static bool _dpp_precsc_enable;
#define SET_PRECSC_ENABLE 0xaaaa   //ccf3.fw's last word, 0xaaaa--> precsc for CCT,
// _dpp_precsc_enable = true; 0x5555, csc2 for CCT
...
static u32 iris_ai_addr[][4] = {
	{0x07041c05, 0xf1700470, 0x04010125, 0xf17003c8},
	{0x07040305, 0xf1740060, 0x04010102, 0xf174006c},
};
...
static long nPreCSCCoffValue[18];
...
void iris_crst_coef_check(const u8 *fw_data, size_t fw_size)
{
	...
	if (iris_crstk_coef_buf[len/2 - 1] == SET_PRECSC_ENABLE)
		_dpp_precsc_enable = true;
	else
		_dpp_precsc_enable = false;

	IRIS_LOGI("csc2 or precsc: 0x%x, precsc_enable: %d\n", iris_crstk_coef_buf[len/2 - 1], _dpp_precsc_enable);
...
void iris_cm_ratio_set(void)
{
	...
	if (!iris_crstk_coef_buf) {
		IRIS_LOGE("invalid iris_crstk_coef_buf");
		return;
	}
	...
	if (_dpp_precsc_enable) {
		...
		uint32_t lTmp1, lTmp2;
		bool sign1, sign2;

		psopt = iris_find_ip_opt(IRIS_IP_DPP, 0x34);  //pre csc' coef
		if (!psopt) {
			IRIS_LOGW("could not find i_p=%d opt_id=0x33", IRIS_IP_DPP);
			return;
		}
		if ((xvalue >= iris_max_x_value) && (xvalue < xvalue_default)) {
			ratio = ((xvalue - iris_max_x_value)*10000)/(xvalue_default - iris_max_x_value);
			for (i = 0; i < 9; i++) {
				if (iris_crstk_coef_buf[coefBuff_start+63+i] > 16383) {
					lTmp1 = 0x8000 - iris_crstk_coef_buf[coefBuff_start+63+i] & 0x7FFF;
					lTmp1 *= ratio;
					sign1 = true;
				} else {
					lTmp1 = iris_crstk_coef_buf[coefBuff_start+63+i] * ratio;
					sign1 = false;
				}
				if (iris_crstk_coef_buf[coefBuff_start+72+i] > 16383) {
					lTmp2 = 0x8000 - iris_crstk_coef_buf[coefBuff_start+72+i] & 0x7FFF;
					lTmp2 *= (10000 - ratio);
					sign2 = true;
				} else {
					lTmp2 = (10000 - ratio)*iris_crstk_coef_buf[coefBuff_start+72+i];
					sign2 = false;
				}

				if (sign1 == false && sign2 == false)
					nPreCSCCoffValue[i] = (lTmp1 + lTmp2)/10000;
				else if (sign1 && sign2) {
					lTmp1 = (lTmp1 + lTmp2)/10000;
					nPreCSCCoffValue[i] = 0x8000 - lTmp1;
				} else if (sign1 && sign2 == false) {
					if (lTmp1 > lTmp2) {
						lTmp1 = (lTmp1 - lTmp2)/10000;
						nPreCSCCoffValue[i] = 0x8000 - lTmp1;
					} else {
						lTmp1 = (lTmp2 - lTmp1)/10000;
						nPreCSCCoffValue[i] = lTmp1;
					}
				} else if (sign1 == false && sign2) {
					if (lTmp1 > lTmp2) {
						lTmp1 = (lTmp1 - lTmp2)/10000;
						nPreCSCCoffValue[i] = lTmp1;
					} else {
			...
			ratio = ((xvalue - xvalue_default)*10000)/(iris_min_x_value - xvalue_default);
			for (i = 0; i < 9; i++) {
				if (iris_crstk_coef_buf[coefBuff_start+54+i] > 16383) {
					lTmp1 = 0x8000 - iris_crstk_coef_buf[coefBuff_start+54+i] & 0x7FFF;
					lTmp1 *= ratio;
					sign1 = true;
				} else {
					lTmp1 = iris_crstk_coef_buf[coefBuff_start+54+i] * ratio;
					sign1 = false;
				}
				if (iris_crstk_coef_buf[coefBuff_start+63+i] > 16383) {
					lTmp2 = 0x8000 - iris_crstk_coef_buf[coefBuff_start+63+i] & 0x7FFF;
					lTmp2 *= (10000 - ratio);
					sign2 = true;
				} else {
					lTmp2 =  iris_crstk_coef_buf[coefBuff_start+63+i]*(10000-ratio);
					sign2 = false;
				}
		...
		IRIS_LOGD("ratio:%d, xvalue: %d, iris_min_x_value: %d", ratio, xvalue, iris_min_x_value);
			...
			data = (uint32_t *)psopt->cmd[0].msg.tx_buf;
			data[2] = nPreCSCCoffValue[1] << 16 | nPreCSCCoffValue[0];
			data[3] = nPreCSCCoffValue[3] << 16 | nPreCSCCoffValue[2];
			data[4] = nPreCSCCoffValue[5] << 16 | nPreCSCCoffValue[4];
			data[5] = nPreCSCCoffValue[7] << 16 | nPreCSCCoffValue[6];
			data[6] = 0x0000 << 16 | nPreCSCCoffValue[8];
			iris_init_update_ipopt_t(IRIS_IP_DPP, 0x34, 0x34, 0x01);
			for (i = 0; i < 9; i++)
				IRIS_LOGD("nPreCSCCoffValue[%d] = 0x%04x", i, nPreCSCCoffValue[i]);
			for (i = 0; i < 27; i++)
				IRIS_LOGD("nPreCSC fw[%d] = 0x%04x", i, iris_crstk_coef_buf[coefBuff_start+54+i]);
		}
	}
...
void iris_cm_colortemp_mode_set(u32 mode, bool bcommit)
{
	...
	if (_dpp_precsc_enable) {
		if (mode && pqlt_cur_setting->pq_setting.cmcolorgamut)
	...
	IRIS_LOGI("cm color temperature mode=%d", mode);
}
...
void iris_cm_color_gamut_pre_set(u32 source_switch)
{
	...
	IRIS_LOGI("source switch = %d", source_switch);
}
...
void iris_dpp_3dlut_gain(u32 count, u32 *values, bool bcommit)
{
	...
	iris_end_dpp(bcommit);
	IRIS_LOGI("3dlut interpolation, gain=0x%x, len=%d", values[0], len);
}
...
void iris_dpp_3dlut_send(u8 lut_optid, uint8_t chain)
{
	...
	IRIS_LOGI("3dlut send optid: 0x%x", lut_optid);
}
...
void iris_cm_color_gamut_set(u32 level, bool bcommit)
{
	...
	case 4:  //sRGB            3d-lut:0xc
	case 5:  //P3              3d-lut:0xd
		interp1_src = level + 8;
		lut3d_interp1 = ((interp1_src << 10) | (interp1_src << 6) | 1);
	...
	case 9:
		interp1_src = 0xa; interp1_src2 = 0xb; interp1_dst = 0x2;
		lut3d_interp1 =  ((interp1_src << 10) | (interp1_src2 << 6) | (interp1_dst << 2) | 1);
		interp2_src = 0x4; interp2_src2 = 0x5; interp2_dst = 0x3;
		lut3d_interp1 |= ((interp2_src << 25) | (interp2_src2 << 21) | (interp2_dst << 17) | (1 << 15));
		interp3_src = 0x3; interp3_src2 = 0x2; interp3_dst = 0;
		lut3d_interp2 = ((interp3_src << 10) | (interp3_src2 << 6) | 1);
		lut3d_interp1 = (payload[0] & 0xc0000000) | lut3d_interp1;
		lut3d_interp2 = (payload[1] & 0xffff8000) | lut3d_interp2;
	break;
	case 10: //Vivid Low Brightness No CAM   3d-lut:0x4
	case 11: //Vidi Low Brightness High CAM  3d-lut:0x5
		interp1_src = level - 0x6;
		lut3d_interp1 = ((interp1_src << 10) | (interp1_src << 6) | 1);
		lut3d_interp1 = (payload[0] & 0xc0000000) | lut3d_interp1;
		lut3d_interp2 = 0;
		lut3d_interp2 = (payload[1] & 0xffff8000) | lut3d_interp2;
...
//#endif
	...
	if (_dpp_precsc_enable)	{
		...
		if ((level > 0) && (pqlt_cur_setting->pq_setting.cmcolortempmode > 0))
			csc_ctrl = csc_ctrl | 0x1 << 8;
		iris_set_ipopt_payload_data(IRIS_IP_DPP, 0x30, 2, csc_ctrl);
		iris_init_update_ipopt_t(IRIS_IP_DPP, 0x30, 0x30, 0x01);
	}
...
void iris_dpp_precsc_enable(u32 enable, bool bcommit)
{
	...
	IRIS_LOGI("dpp precsc enable =%d", enable);
}
...
static void iris_sdr2hdr_ai_addr_switch(u32 value)
{
	uint32_t  *payload = NULL;
	int i;
	int level = 0;

	if (value == SDR2HDR_Bypass)
		level = 1;

	payload = iris_get_ipopt_payload_data(IRIS_IP_AI, 0x40, 2);
	for (i = 0; i < 4; i++)
		payload[2*i] = iris_ai_addr[level][i];

	iris_init_update_ipopt_t(IRIS_IP_AI, 0x40, 0x40, 0x01);
	iris_init_update_ipopt_t(IRIS_IP_AI, 0x90, 0x90, 0x01);
}
...
void iris_pwil_dport_disable(bool enable, u32 value, bool update)
{
	uint32_t  *payload = NULL;
	struct iris_cfg *pcfg = iris_get_cfg();
	...
	payload[0] &= 0x03ffffff;
	payload[0] |= (value << 27);
	if (enable)
		payload[0] |= 0x04000000;

	iris_init_update_ipopt_t(IRIS_IP_PWIL, 0xd0, 0xd0, 1);
...
void iris_sdr2hdr_level_set(u32 level)
{
	...
	iris_sdr2hdr_ai_addr_switch(level);
	...
	IRIS_LOGW("sdr2hdr level =%d", level);
}
...
void iris_scaler_gamma_enable(bool lightup_en, u32 level)
{
	...
	len = iris_update_ip_opt(IRIS_IP_DPP,
			level, chain);
...
void iris_hdr_csc_complete(int step)
{
	...
	if (!iris_dynamic_power_get()) {
		...
		if (sdr2hdr_level_orig == 0 &&
			...
			pqlt_cur_setting->pq_setting.sdr2hdr == 0) {
			iris_dma_trig(DMA_CH10 | DMA_CH12, 0);
			iris_update_pq_opt(iris_pq_update_path, true);
		} else {
...
static void iris_brightness_para_set(uint32_t *values)
{
	...
	IRIS_LOGI("dimming set: enable %d, values: 0x%x\n", dimmingEnable, values[1]);
	regval.ip = IRIS_IP_DPP;
