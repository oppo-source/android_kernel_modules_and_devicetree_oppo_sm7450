...
#define LOG_DEBUG_INFO			((log_level >= 1) || IRIS_IF_LOGD())
#define LOG_VERBOSE_INFO		((log_level >= 2) || IRIS_IF_LOGV())
#define LOG_VERY_VERBOSE_INFO	((log_level >= 3) || IRIS_IF_LOGVV())
...
static void _iris_init_param(struct device_node *np)
{
	...
	if (panel_tm_num > 1) {
		...
		master_tm_num = of_property_count_u8_elems(np, "pxlw,master-timing-cmd-map");
		if (master_tm_num > 0) {
...
static int32_t _iris_parse_master_timing_cmd_map(struct device_node *np)
{
	...
	rc = of_property_read_u8_array(np, "pxlw,master-timing-cmd-map",
...
int32_t iris_parse_timing_switch_info(struct device_node *np)
{
	...
	for (cmd_idx = IRIS_DTSI_PIP_IDX_START + 1; cmd_idx < iris_get_cmd_list_cnt(); cmd_idx++) {
		...
		const struct dsi_mode_info *new_timing)
{
	...
	IRIS_LOGI("%s(), new timing: %ux%u@%u, timing@%u, new cmd list index: %u, iris current mode '%s'",
			__func__,
			new_timing->h_active,
			new_timing->v_active,
...
static void _iris_sync_blending(void)
{
	...
	if (pcfg->dual_enabled && pcfg->frc_enabled)
		iris_send_ipopt_cmds(IRIS_IP_BLEND, 0x20);
...
static void _iris_switch_fps_impl(uint32_t new_fps)
{
	ktime_t ktime = 0;
	...
	//iris_send_ipopt_cmds(IRIS_IP_TX, ID_TX_BYPASS_CTRL);

	if (LOG_DEBUG_INFO)
		ktime = ktime_get();

	iris_set_out_frame_rate(new_fps);
	IRIS_LOGI_IF(LOG_DEBUG_INFO,
			"%s(), switch DTG cost '%d us'", __func__,
			(u32)(ktime_to_us(ktime_get()) - ktime_to_us(ktime)));
}
		...
		struct dsi_mode_info *new_timing)
{
	...
	IRIS_LOGI("%s(), new timing: %ux%u@%uHz, current cmd list index: %u, switch case: %s",
			__func__,
			new_timing->h_active,
			new_timing->v_active,
			new_timing->refresh_rate,
	...
	if (lightup_opt & 0x8) {
		...
		IRIS_LOGI("%s(), force ABYP mode", __func__);
		SDE_ATRACE_END(__func__);
		return rc;
	}
...
static void _iris_switch_proc(struct dsi_mode_info *new_timing)
{
	...
	if (!_iris_is_same_fps(new_timing, last_pt_timing))
		_iris_switch_fps_impl(new_timing->refresh_rate);
}
