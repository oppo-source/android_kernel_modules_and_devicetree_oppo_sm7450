...
bool iris_emv_pipelines_idle(void)
{
...
{
	pr_err("iris test PWIL_0_CTRL0 = %x\n", iris_pwil_0_ctrl0_get());
	pr_err("iris test PWIL_0_INPUT_META = %x\n", iris_pwil_0_input_meta_ctrl_get());
	pr_err("iris test PWIL_0_disp_ctrl0 = %x\n", iris_pwil_0_disp_ctrl0_get());
}
...
u32 iris_emv_pwil_video_ctrl7_get(void)
{
	u32 *payload = NULL;
...
{
	u32 addr = iris_emv_pwil_video_ctrl7_get();
	u32 cmd[2];
	u32 ctrl[1];
	u32 *payload = NULL;

	addr = (emv) ? 0x000a0000 : addr;
...
u32 iris_emv_pwil_mv1buffer_base_addr_get(void)
{
	...
	addr = payload[0];
	IRIS_LOGE("%s: MV1_BASE_ADDR2 = 0x%x", __func__, addr);
	if (addr == 0)
		addr = 0x00271000;
...
void iris_emv_pwil_mvdbuffer_alloc(bool lli, bool batch)
{
	...
	IRIS_LOGE("%s: MV1ADDR2 = 0x%x[0x%x][0x%x][0x%x], offset = 0x%x(mvd:%dx%d)",
		__func__,
		mvstart,
		mvstart + offset,
		mvstart + 2*offset,
...
void iris_emv_blending_flush_ctrl(bool video, bool graphic, bool osd, bool batch)
{
	...
	IRIS_LOGE("%s:flush video %d, graphic %d, osd %d, 0x%x, csrto 0x%x(0x%x)", __func__,
					...
					bool lli, bool batch, bool emvEnter)
{
	...
	IRIS_LOGE("%s: dsc_bpp 0x%x, hstride %d", __func__, dsc_bpp, hstride);
...
void iris_emv_pwil_configure_mv_ctrl(int videoWidth, int videoHeight, bool lli, bool batch)
{
	...
	IRIS_LOGE("%s: bmv %dx%d", __func__, bmvSize, bmvSize);
...
void iris_emv_pwil_shortpacket_mask(int enable)
{
	...
	IRIS_LOGE("%s: mask = %d", __func__, enable);
}
...
void iris_emv_exit(void)
{
	...
	//vid_hres = (vid_proc > 0) ? vid_hres : pcfg->frc_setting.mv_hres;
	//vid_vres = (vid_proc > 0) ? vid_vres : pcfg->frc_setting.mv_vres;

	IRIS_LOGE("%s:in [%dx%d][%dx%d][%dx%d][%dx%d]", __func__,
...
void irisSetExtMvFrc(struct extmv_frc_meta meta)
{
	...
	if (meta.mode == 0xECEA) {
		...
		if (meta.gameWidth*meta.gameHeight > iris_emv_limit) {
			...
			meta.overflow = 0xe0;
			IRIS_LOGE("i7:META: mvdH=%d,mvdW=%d,BlockMax=%d,BlockSize=%d, frc %dx%d",
			mvdHeight, mvdWidth,
		...
		IRIS_LOGE("META:src %dx%d,pa:%d,%d,gmvd:%d,%d,%dx%d,game:%d,%d,%dx%d,ov 0x%x,mv0:%d,%d,mv1:%d,%d,%dx%d",
...
void iris_emv_perf_dump(void)
{
	struct iris_cfg *pcfg = iris_get_cfg();
	uint32_t timeus0 = 0;

	if ((pcfg == NULL) || (!pcfg->emv_perf.valid))
		return;

	timeus0 = (u32)ktime_to_us(pcfg->emv_perf.kt[kt_iris2nd_up_ready]);
	timeus0 -= (u32)ktime_to_us(pcfg->emv_perf.kt[kt_perf_start]);
}
...
void iris_emv_on_mipi1_up(void)
{
	if (iris_emv_game_mode_enabled()) {
		...
		IRIS_LOGE("%s: capt_en close pipe 1", __func__);
...
bool iris_emv_on_dual_on(bool success)
{
	...
	if ((iris_emv_mode_get() == IRIS_EMV_ON_SWAP) && (timeout > 0)) {
		pr_err("iris test GET_IRIS_PIPELINES IDLEs for swap channels %d\n", timeout);
		timeout--;
		return iris_emv_pipelines_idle();
	}
	return true;
}
...
void iris_emv_mode_switch(int mode)
{
	...
	IRIS_LOGE("%s:from mode %d [shape %d] into mode %d", __func__,
...
{
	IRIS_LOGE("%s: in shape %d", __func__, iris_emv_shaped);
...
{
	IRIS_LOGE("%s: in touched %d [%d %d %d, %d %d]", __func__,
