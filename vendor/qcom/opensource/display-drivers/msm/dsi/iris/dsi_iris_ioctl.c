...
static int mdss_mipi_dsi_command(void __user *values)
{
	...
	desc.ctrl_flags |= ((cmd.flags & MSMFB_MIPI_DSI_COMMAND_ACK) > 0 ? DSI_CTRL_CMD_READ : 0);
	desc.post_wait_ms = 0;
	desc.msg.ctrl = 0;
	if (cmd.dtype == 0x0f) {
		if (!desc.msg.tx_buf) {
	...
	if (cmd.flags & MSMFB_MIPI_DSI_COMMAND_ACK)
		desc.ctrl_flags = desc.ctrl_flags | DSI_CTRL_CMD_READ;
...
static int _iris_configure(u32 display, u32 type, u32 value)
{
	...
	case IRIS_ANALOG_BYPASS_MODE:
		...
		iris_abyp_switch_proc(pcfg->display, value);
		break;
	...
	case IRIS_PWIL_DPORT_DISABLE:
		...
		iris_pwil_dport_disable(value, 2, true);
		break;
...
static int _iris_configure_ex(u32 display, u32 type, u32 count, u32 *values)
{
	...
	case IRIS_DPP_3DLUT_GAIN:
		iris_dpp_3dlut_gain(count, &values[0], true);
		break;
...
int iris_configure_get(u32 display, u32 type, u32 count, u32 *values)
{
	...
	case IRIS_PANEL_TE:
		*values = 0;
		...
		IRIS_LOGI("get IRIS_PANEL_TE: %d, iris init %d, panel init %d",
			pcfg->panel_te, pcfg->iris_initialized,
...
void iris_debug_info_get(u32 *value, u32 count)
{
	...
	case 0:
		...
		break;
	case 1:
		len = iris_debug_display_mode_get(buf, count);
		break;
	case 2:
