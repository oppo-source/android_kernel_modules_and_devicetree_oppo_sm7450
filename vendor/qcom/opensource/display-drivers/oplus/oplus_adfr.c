		...
		int ctrl_idx, u32 extend_frame)
{
	...
	if (!strcmp(panel->oplus_priv.vendor_name, "S6E3HC4")) {
		high = extend_frame*3/256;
		extend_frame = extend_frame*3%256;
	}
	DSI_INFO("kVRR extend_frame = %d\n", extend_frame);
	...
	if (oplus_adfr_auto_mode == OPLUS_ADFR_AUTO_OFF) {
		...
		if (!strcmp(panel->oplus_priv.vendor_name, "S6E3HC4")) {
			if (count <= SDC_MANUAL_MIN_FPS_CMD_HIGH_OFFSET) {
				DSI_ERR("kVRR [%s] No commands to be sent for manual min fps high, wrong cmds count.\n", panel->name);
				goto exit;
			}
			/*update manual min fps*/
			...
			if (tx_len != SDC_MIN_FPS_CMD_SIZE) {
				DSI_ERR("kVRR [%s] No commands to be sent for manual min fps high, wrong cmds size %u.\n", panel->name, tx_len);
				goto exit;
			}

			tx_buf[SDC_MIN_FPS_CMD_SIZE-1] = high;
		}
		...
		if (!strcmp(panel->oplus_priv.vendor_name, "S6E3HC4")) {
			if (count <= SDC_AUTO_MIN_FPS_CMD_HIGH_OFFSET) {
				DSI_ERR("kVRR [%s] No commands to be sent for auto min fps high, wrong cmds count.\n", panel->name);
				goto exit;
			}
			/*update auto min fps*/
			...
			if (tx_len != SDC_MIN_FPS_CMD_SIZE) {
				DSI_ERR("kVRR [%s] No commands to be sent for auto min fps high, wrong cmds size %u.\n", panel->name, tx_len);
				goto exit;
			}
