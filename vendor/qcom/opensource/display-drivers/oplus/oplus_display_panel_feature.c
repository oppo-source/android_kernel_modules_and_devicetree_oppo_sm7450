...
void oplus_panel_update_backlight(struct mipi_dsi_device *dsi, u32 bl_lvl)
{
	...
	if (!rc && !oplus_dimlayer_hbm &&
			!strcmp(dsi_display->panel->oplus_priv.vendor_name, "AMS662ZS02")) {
		if ((bl_lvl >= dsi_display->panel->bl_config.bl_max_level) &&
				(8 != seed_mode))
			dsi_panel_seed_mode(dsi_display->panel, 8);
		else if (((bl_lvl < dsi_display->panel->bl_config.bl_max_level) &&
				(8 == seed_mode)) || (100 == seed_mode))
			dsi_panel_seed_mode(dsi_display->panel, 0);
	}
}
